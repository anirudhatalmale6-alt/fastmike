...
/* ===========================================================================
 * CPU tone engine - the safety net.
 *
 * A WebGL context can go away at any time: a driver update, a laptop waking
 * from sleep, running over remote desktop, or a machine with no usable GPU at
 * all. Chromium fires webglcontextlost and from then on every draw silently
 * does nothing - which would mean blank previews and solid black prints with
 * no error anywhere. So there is a second engine that produces byte-identical
 * output on the CPU. It is slower, but the app never stops working.
 * ======================================================================== */
...
class CpuRenderer {
  ...
  draw(rect, rot, adj) {
    ...
    // rect is already in display orientation, so undo the swap before drawing
    const odd = ((rot % 4) + 4) % 4 % 2 === 1;
    const dw = odd ? rect.h : rect.w;
    const dh = odd ? rect.w : rect.h;
  ...
  /** Same maths as the fragment shader, applied over the drawn region only. */
  _tone(rect, adj) {
...
/* ===========================================================================
 * Surface - owns a canvas and whichever engine is currently healthy.
 *
 * A canvas that has ever handed out a WebGL context can never return a 2D one,
 * so dropping to the CPU engine means swapping in a fresh canvas element.
 * ======================================================================== */
...
class Surface {
  ...
  draw(rect, rot, adj) {
    this.engine.draw(rect, rot, adj);
    // A lost context reports no error and draws nothing, so check explicitly
...
const app = {
  ...
  imageOf: null,     // which photo id `image` belongs to
  loadToken: 0,
 ...
 *
 * On the desktop we deliberately keep only the file path plus a small
 * thumbnail in memory and re-read the original from disk when it is actually
 * needed. Holding every full-size original would put a 200-photo shoot into
 * the gigabytes; this keeps it flat.
 */
...
function layoutFrame() {
  ...
  tone.canvas.width = Math.round(sw * dpr);
  tone.canvas.height = Math.round(sh * dpr);
}
...
async function addPhotos(entries) {
  ...
  for (const entry of entries) {
    try {
      // decoded once here for the thumbnail, then dropped - see srcFor()
      const src = entry.path && DESKTOP ? await window.fastmike.readImage(entry.path) : entry.url;
...
async function importFiles() {
  if (DESKTOP) {
    await addPhotos(await window.fastmike.importFiles());
  } else {
    el.webPicker.click();
  }
}
...
async function importFolder() {
  if (DESKTOP) {
    await addPhotos(await window.fastmike.importFolder());
  } else {
...
async function selectPhoto(i) {
  ...
  try {
    const img = await loadImage(await srcFor(p));
...
function renderToPrint(photo, img) {
  ...
  exportTone.canvas.width = out.w;
  exportTone.canvas.height = out.h;
  ...
  return new Promise((resolve) => {
    exportTone.canvas.toBlob((b) => resolve(b), 'image/jpeg', 0.95);
  });
}
...
async function addToEdited() {
  ...
  // reuse the already-decoded preview image when it is the one on screen
  const img = app.image && app.imageOf === p.id ? app.image : await loadImage(await srcFor(p));
...
document.addEventListener('drop', async (e) => {
  ...
  await addPhotos(files.map((f) => {
    const p = DESKTOP ? window.fastmike.pathForFile(f) : null;
    return p ? { name: f.name, path: p } : { name: f.name, url: URL.createObjectURL(f) };
  }));
});
...
window.__fastmikeInternals = { tone, exportTone, renderToPrint, photoRect, printPixels };
